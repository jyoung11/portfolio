...
namespace Drupal\Tests\ebt_video_and_image_gallery\Functional;
 ...
 *
 * @group ebt_core
 * @group ebt_video_and_image_gallery
 */
class InstallTest extends BrowserTestBase {
  ...
  public function testInstallation() {
    ...
    $media_type = $this->createMediaType('oembed:video', ['id' => 'remote_video']);
    $media_type_id = $media_type->id();
    $media_type->setFieldMap(['name' => 'name']);
    $media_type->save();
    ...
    // Disable the alt text field, because this is not a JavaScript test and
    // the alt text field will therefore not appear without a full page refresh.
    $field = FieldConfig::load("media.$media_type_id.field_media_oembed_video");
    $settings = $field->getSettings();
    $settings['max_length'] = 255;
    $settings['case_sensitive'] = FALSE;
    $settings['is_ascii'] = FALSE;
    $field->set('settings', $settings);
    $field->save();

    $this->assertFalse($this->moduleHandler->moduleExists('ebt_video_and_image_gallery'));
    ...
    $this->assertTrue($this->moduleInstaller->install(['ebt_video_and_image_gallery']));
    \Drupal::service('config.installer')->installDefaultConfig('module', 'ebt_video_and_image_gallery');
    $this->reloadServices();
    $this->assertTrue($this->moduleHandler->moduleExists('ebt_video_and_image_gallery'));
