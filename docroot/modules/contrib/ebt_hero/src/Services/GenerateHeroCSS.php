...
class GenerateHeroCSS implements ContainerInjectionInterface {
  ...
  public function generateFromSettings($settings, $block_class) {
    $hero_selector = '.' . $block_class . ' .ebt-hero-container';
    $col_1_selector = '.' . $block_class . ' .ebt-hero-container .hero-col-1';
    $col_2_selector = '.' . $block_class . ' .ebt-hero-container .hero-col-2';
    $hero_styles = '';

    $mobile_breakpoint = $settings['mobile_breakpoint'] ?? $this->config->get('ebt_core_mobile_breakpoint');
    ...
    if (!empty($settings['overlay'])) {
      ...
      $hero_styles .= '.' . $block_class . ' { ';
      $hero_styles .= ' position: relative; ';
      $hero_styles .= ' } ';

      $hero_styles .= '.' . $block_class . ' .ebt-container { ';
